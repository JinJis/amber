    ...
    ConnectorManifest(
        ...
        resources=[
            ...
            *_statements("US", PROV_SEC, CostTier.free),
            Resource(name="filings",
                     description="SEC filings list. Unfiltered = raw recent submissions (insider Form 4s "
                                 "dominate); pass filing_type for the report you actually want.",
                     path="/filings", output_model="FilingsResponse",
                     markets=["US"], cost_tier=CostTier.free,
                     params=[P_TICKER, P_FILING_TYPE_US, P_LIMIT, P_MARKET], provenance=PROV_SEC_FILINGS),
    ...
    ConnectorManifest(
        ...
        resources=[
            ...
            Resource(name="interest_rates_snapshot", description="Latest interest rate.", path="/macro/interest-rates/snapshot",
                     output_model="InterestRatesResponse", markets=["US"], cost_tier=CostTier.low,
                     params=[ResourceParam(name="bank", required=True, enum=["FED", "ECB", "BOE", "BOJ"]), P_MARKET], provenance=PROV_FRED),
            Resource(name="economic_indicators", description="Economic indicators (CPI·unemployment·GDP·환율 …) — US/EA는 DBnomics·BLS, 한국(KR)은 한국은행 ECOS. 그룹(하위요인)·지역 분류, 카탈로그 열람.",
                     path="/macro/indicators", output_model="EconomicIndicatorsResponse", markets=["US", "KR"], cost_tier=CostTier.low,
                     params=[ResourceParam(name="indicator",
                                           enum=["cpi", "core_cpi", "pce_price", "semiconductor_ppi", "euro_cpi",
                                                 "unemployment", "nonfarm_payrolls", "labor_participation",
                                                 "gdp_growth", "industrial_production", "treasury_10y", "treasury_3m",
                                                 "kr_cpi", "kr_ppi", "kr_unemployment", "kr_gdp", "kr_usdkrw"],
                                           description="Indicator slug — 반드시 이 목록에서 선택. 한국: kr_cpi(소비자물가)·kr_unemployment(실업률)·kr_gdp(실질GDP)·kr_ppi·kr_usdkrw(환율); omit to list all."),
                             ResourceParam(name="region", description="카탈로그 지역 필터 (US/EA/KR)."),
            ...
            Resource(name="macro_panel", description="국가경제 패널 — 한 지역의 핵심 거시지표 최신값·직전·변화 스냅샷(그룹별). US/EA=DBnomics·BLS, 한국=한국은행 ECOS.",
                     path="/macro/panel", output_model="MacroPanelResponse", markets=["US", "KR"], cost_tier=CostTier.low,
                     params=[ResourceParam(name="region", description="국가/지역 (US, EA, KR).")],
                     provenance=PROV_DBNOMICS),
        ],
    ),
    ConnectorManifest(
        ...
        resources=[
            ...
            *_statements("KR", PROV_DART, CostTier.low),
            Resource(name="filings",
                     description="DART filings list. Unfiltered = raw recent submissions; pass "
                                 "filing_type for the report you actually want.",
                     path="/filings", output_model="FilingsResponse",
                     markets=["KR"], cost_tier=CostTier.low,
                     params=[P_TICKER, P_FILING_TYPE_KR, P_LIMIT, P_MARKET], provenance=PROV_DART_FILINGS),
    ...
    ConnectorManifest(
        ...
        resources=[
            ...
            Resource(name="backtest",
                     ...
                     params=[ResourceParam(name="holdings", required=True,
                                           description='보유 종목·비중의 JSON 배열 문자열. 예: [{"ticker":"AAPL","weight":0.6},{"ticker":"MSFT","weight":0.4}] (weight 합=1)'),
                             ResourceParam(name="start_date", description="시작일 YYYY-MM-DD (생략 시 저장 구간 전체)."),
                             ResourceParam(name="end_date", description="종료일 YYYY-MM-DD."),
                             ResourceParam(name="benchmark", description="벤치마크 티커 (예: SPY)."),
                             P_MARKET],
                     provenance=Provenance(source="ingestion store (Yahoo prices)", freshness=Freshness.eod)),

...
_SYNTHESIS_PROMPT = (
    ...
    "- 핵심 강조(정확히 한 번): 답변 전체에서 독자가 가장 먼저 읽어야 할 '한 문장 또는 짧은 구절' "
    "하나만 골라 `==…==`로 감싸세요 (보통 리드의 핵심 답 문장). 답변당 딱 1회, 한 문장을 넘기지 "
    "말고, 헤딩·표·그림 마커에는 쓰지 마세요. 이 표시는 '핵심(먼저 읽을 곳)' 전용입니다 — 수치 강조가 "
    "아니며(수치는 [n] 인용으로 충분), `==`를 두 개(여는·닫는) 짝으로만 쓰세요.\n"

...
최신 기록 스니펫들입니다([n] 인덱스). 이 종목을 지켜보는 사용자가 "오늘 물어볼 만한" 깊이 있는
...
- 얕은 질문("주가 알려줘") 금지 — 스니펫의 구체 사실에서 출발하는 파고드는 질문일 것.
  좋은 예: "이번 10-Q에서 재고자산이 전분기 대비 어떻게 움직였나?", "이번 낙폭이 과거 분포에서
  몇 퍼센타일인가?", "최신 공시의 신규 위험요소는 무엇인가?"
- hook: 스니펫의 실제 사실 한 줄 (수치·날짜는 스니펫 그대로; 지어내지 말 것) — 왜 지금 이
  질문인지의 근거.
- question: 우리 도구로 사실 조회해 답할 수 있는 구체적 질문 (한국어 1문장). 해석·의견·전망
  요구("어떻게 봐야", "전망은") 금지.
...
- question: 그 트렌드를 우리 도구로 더 파볼 수 있는 사실 조회형 질문 (한국어 1문장).
  해석·전망 요구 금지.

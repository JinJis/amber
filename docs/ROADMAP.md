...
| agent-engine | 111 | 200 (measured — ENTRY-v6 +3: ask-feed 섹션 plan/dispatch/kind-filter) | ✅ DK-1 (+5); then ≥10 HL-6/7, ≥8 QT-2 (number audit), ≥8 EC-3, ≥6 HL-9 |
| studio-api | 40 | 136 (measured — ENTRY-v6 +3: 섹션 refresh/assemble/refresh-sections 가드) | ✅ FLAG-1 scheduler gate (+1), ✅ DK-3 (+4); then ≥7 HL-12/14 BFF |
...
| eval scenarios | 32 | 95 (scenarios.py — ENTRY-v6 +1: 어닝 레이더 섹션 카드 무출처 방지, `kind: ask_feed` runner) | ✅ DK-4 (+2 desk-feed, `kind: desk_feed` runner); then +4 HL-6, +2 QT-2, +1 EC-3, +2 M2 flows, +1 FI |

...
| **ING-1** | RAG 인제스트 원자성·배치 개편 ([RAG_INGEST_BATCHING_SPEC](./RAG_INGEST_BATCHING_SPEC.md)) | 대형 공시 인제스트를 "빠르고·안 깨지고·재시도 무비용"으로: replace-스코프당 단일 POST · rag 원자적 prune-swap(`replace_scope`, advisory lock, 임베딩 후 단일 txn — 잘린 공시 서빙/접수번호 소실 창 제거) · replace tenant 고정 · 임베딩 동시성(RAG_EMBED_CONCURRENCY, 실측 436s/배치 → ~¼) · 문서수 비례 클라 타임아웃+1회 재시도 · 아이템 단위 델타 커서 · accession 인덱스. 7페이즈, rag 먼저 배포 | OPS-2 | ⬜ planned (스펙 완성 2026-07-12; AAPL 잘린 접수번호 2건은 배포 후 델타 스윕이 자연 치유 — 스펙 §4로 증명) |
...
|---|---|---|---|
| datasets | 148 | 332 (measured — EV-FIX +42 · OPS-2 델타/커서/쿼터 +16) | ✅ OPS-1 (+2 grouping/runner); then ≥32 HL-1/2/3, ≥12 HL-4, ≥9 HL-5, ≥22 QT-1/4, ≥7 EC-1, ≥14 FI-1/2/3, ≥8 HL-8/EC-2 |
...
| rag | 20 | 28 | ≥4 HL-5 (era_news/dossier doc types), ≥2 FI-1 (section filter) |

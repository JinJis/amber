...
> 원자성·배치 개편(ING-1)은 [`docs/RAG_INGEST_BATCHING_SPEC.md`](./RAG_INGEST_BATCHING_SPEC.md).
...
| admin | — | 28 (measured — OPS-2 어드민 개편 +8: /runs·쿼터 카드·mode 전달) | UI 회귀 가드 (렌더 스모크) |
| agent-engine | 111 | 184 (measured — EV-FIX: financials 언랩·인용 dedup 병합·실패콜 스킵 +15) | ✅ DK-1 (+5); then ≥10 HL-6/7, ≥8 QT-2 (number audit), ≥8 EC-3, ≥6 HL-9 |
...
| web | TS build only | 85 (vitest — EV-FIX: toCitation 보존·DerivationCard 우선·FilingViewer 폴백 +17) | component tests for DK-2 (3 states), HL-7/10/11/12/13, QT-3 (scatter/distribution/계산 근거), EC-4 |
| eval scenarios | 32 | 94 (scenarios.py — EV-FIX +1: US 사업보고서 근거 앵커) | ✅ DK-4 (+2 desk-feed, `kind: desk_feed` runner); then +4 HL-6, +2 QT-2, +1 EC-3, +2 M2 flows, +1 FI |

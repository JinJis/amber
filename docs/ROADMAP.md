...
| **FEED-FIX** | 어닝 레이더 미생성 수정 (2026-07-19) | 어닝 레이더가 아예 0장이던 근본 원인: **fmp가 PREMIUM_CONNECTORS라 백그라운드 시스템 피드 키에 미활성** → `fmp__earnings_calendar`가 엔타이틀 카탈로그에 없어 플랜이 비었음(guru=sec_edgar·history=market_history는 FREE라 정상). ① 시스템 피드 프로젝트가 **카탈로그 전체**를 엔타이틀(유저 플랜 아님 — FREE/PREMIUM 플랜 분류와 무관하게 fmp·kis·gdelt·nyt까지 전 커넥터; `_all_connector_ids`가 카탈로그를 소스오브트루스로 fetch, 실패 시 폴백) + **기존 프로젝트는 재시작 시 백필로 자가치유** ② earnings 플랜에 **키불요 SEC 폴백**(`sec_edgar__earnings` XBRL actuals) — fmp 키/활성 없어도 과거 EPS로 생성; 프롬프트를 data-shape 중립적으로(발표일 없으면 추이/서프라이즈만) | — | ✅ agent-engine 204·studio 144 |

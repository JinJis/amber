...
| **FEED-FIX / SYS-1** | 어닝 레이더 미생성 → 시스템 프로젝트 = internal 클래스 (2026-07-19) | 어닝 레이더가 0장이던 근본: **백그라운드 시스템 피드 프로젝트가 자기 엔타이틀을 free 유저처럼 제한**(fmp 미활성) → plan 빔(guru=sec_edgar·history=market_history는 FREE라 정상). 처음엔 "시스템에 전체 카탈로그 activate"로 고쳤으나, 이는 여전히 인프라에 커머셜 activation 목록을 씌우는 것(드리프트·백필 코드 상존). **SYS-1**: control-plane에 `Project.internal` 도입 — 게이트웨이가 internal 프로젝트는 **엔타이틀(activation) 체크만 건너뜀**(카탈로그 전체 엔타이틀), auth·rate-limit·meter·audit은 그대로(불변조건 #2 유지). internal은 admin PATCH(서버)로만 세팅(스푸핑 불가). studio는 시스템 프로젝트를 internal로 표시하고 activation 안 씀(`_all_connector_ids`·카탈로그 싱크 삭제) + 기존 배포는 재시작 시 internal 백필 자가치유. 커머셜 플랜은 그대로 유저별(apply_plan). ② earnings 플랜 **키불요 SEC 폴백**(`sec_edgar__earnings` XBRL actuals) — 프롬프트 data-shape 중립 | — | ✅ control-plane 23·studio 146·agent-engine 204 |
| **SIMPL-1** | 계정 모델 단순화 — Tenant 계층 제거 (2026-07-19) | 유저 지적: tenants→projects→api_keys→activations가 너무 얽혀 있다. 감사 결과 **Tenant는 죽은 계층**(유저당 항상 1:1, `tenant_id`가 데이터플레인에 도달하는 곳 0, 소비처는 admin 원가뷰의 이름 라벨=이메일 하나뿐; 게스트는 N명이 1 tenant/project를 공유해 정체성 계층도 아님). **Project가 진짜 계정 정체성**(인증·엔타이틀·rate·internal·계측·감사·LLM원가·RAG 격리가 전부 project_id 키; RAG의 `X-Tenant-Id`도 값은 project_id). teams/org 로드맵도 없음. → **Tenant 엔티티·`Project.tenant_id`·`User.tenant_id` 전부 제거**, Project.name=오너 이메일(라벨 승계). 프로비저닝 3콜→2콜(`POST /admin/projects` top-level). ApiKey·Activation은 유지(각각 자격증명·엔타이틀로 존재 이유 있음). 게이트웨이 인증경로는 이미 project_id로 붕괴돼 무영향. forward-migration이 기존 DB에서 tenant_id 컬럼(인덱스 먼저 드롭)+tenants 테이블 제거(프로드 업그레이드 검증: 행 보존·신규 insert OK). admin 패널·ARCHITECTURE.md 동기화 | FEED-FIX/SYS-1 | ✅ control-plane 31·studio 146·admin 72 |
| **DBG-1** | 답변 실패 디버그 (2026-07-19) | 답변 생성 실패 시 원인을 챗 UI에서 볼 수 있게: agent-engine stream_chat·studio run-driver 두 실패 경로가 예외 상세를 **답변 텍스트가 아닌 structured `debug` SSE 이벤트**(where·detail·traceback)로 emit → 프런트가 Msg.debug에 담아 답변 액션 케밥에 **에러 있을 때만 '🐞 디버그' 항목**(Modal에 위치·오류·트레이스백+전체 복사) 노출. 사용자 대면 토큰은 상세 없이 친근하게. **studio run-driver가 그동안 예외를 로깅조차 안 하던 갭 수정**(logger.exception — 빈번한 실패가 이제 studio-api 로그에 남음) | — | ✅ agent-engine 203·studio 144·web 99 |
| **INFRA-1** | GCP 배포 자산 ([INFRA](./INFRA.md), 2026-07-17) | 단일 GCE VM(asia-northeast3, e2-highmem-4 32GB) + Cloudflare 무료 프록시 → Caddy(Origin CA·SSE flush) 아키텍처 확정 · `docker-compose.prod.yml`(caddy·전 API 포트 루프백 강제·mem_limit 예산·worker 헬스체크 — ME-13 일부 해소) · `deploy/`(provision.sh: VM·고정IP·IAP-only SSH 방화벽·GCS 백업 버킷 30d 라이프사이클·스냅샷 스케줄 7d / vm-setup.sh / backup.sh+systemd 타이머 — **CR-11 백업 해소**) · web Dockerfile `NEXT_PUBLIC_TOSS_CLIENT_KEY` ARG(프로드 빌드 키 누락 버그 수정) · 비용 모델: 베타 ~$213–245/mo(온디맨드)→$150–183(1-yr CUD)+Gemini 토큰 별도 · 관리형 DB(Cloud SQL PITR·AlloyDB)는 트리거 기반 성장 경로 · **스타터 티어**(e2-standard-4 16GB, `VG_MEM_*` 프로필, −$44/mo — 첫 유저용, `set-machine-type`으로 3–5분 다운타임 1회 승격) · **무중단 롤링 배포 `deploy/deploy.sh`**(pull/빌드 먼저→서비스별 `--wait` 헬스 게이트→studio/agent `stop_grace_period 45s` SSE 드레인→Caddy `lb_try_duration` 웹 스왑 갭 흡수; pg는 재기동 제외) · **Artifact Registry 기본**(서울 `vg` 레포, `build-push.sh`가 개발 머신에서 빌드·푸시(태그=SHA), VM은 pull만; 클린업 정책 최근 10버전·30일로 스토리지 상한 ~2–4GB; 롤백=이전 태그 pull) · `resize-vm.sh`(머신타입 변경, 다운 ~3–5분) · 자원 무증식: 전 리소스 describe∥create 가드 + AR/GCS/스냅샷/로그 전부 보존 상한 | M-SCALE | ✅ 자산 커밋 — 프로비저닝 실행은 오너(USER_TODO §1-12) |
| **BRAND-1** | Amber 리브랜드 ([docs/branding/](./branding/) — 로고·brand.css가 소스 오브 트루스, 경로 데이터 수정 금지) | 서비스명 ValueGraph→**Amber** + 로고·컬러·타이포 전면 적용: ① `web/app/brand.css`(+data-theme 배선)·`web/components/Logo.tsx` 배치, globals.css 전 토큰을 brand 토큰 참조/color-mix 파생으로 재매핑(새 hex 금지), 그림자·글로우 제거, 마스코트 스프라이트→Logo 마크(ui.tsx Mascot 위임) ② 파비콘 SVG+PNG(32/180/192/512)+site.webmanifest(theme #412402)+기본 OG 1200×630(resvg 생성)+`web/public` Dockerfile COPY ③ 폰트 2종: Inter Tight(display)+Pretendard(dynamic-subset CDN), Space Grotesk/Mono 제거 ④ `.evidence-highlight`=LG-4 검증 수치(하단 보더 load-bearing) ⑤ OG 라우트·캔버스 공유카드 웜 팔레트+마크+소문자 워드마크(Inter Tight TTF 번들) ⑥ 유저 대면 문자열 전수 리네임(웹 15파일·studio 8·admin 3; 내부 식별자·UA·도커/패키지명 유지) | — | ✅ 2026-07-19 — web 95·studio 142·admin 70 green + next build 통과 |
| **BRAND-2** | finnote 리브랜드 (Amber→**finnote**, finance+footnote; [디자인 템플릿](./finnote-design-template_final.html)=명세, [docs/branding/](./branding/)=소스 오브 트루스, 로고 경로 데이터 수정 금지) | 서비스명 Amber→**finnote** + 수면 위 지느러미 로고·오션/심해/하이라이터 팔레트·역할별 타이포(Nunito=로고·Pretendard=UI/답변·본명조=원문 인용) 전면 적용: ① `brand.css` 토큰 전면 교체(ink/ocean/hl/paper·rule + space/radius/motion/elevation/wave)·globals.css `:root` 시맨틱 리매핑(`--amber-*` 전 참조 제거, 오션=조작·노랑=근거 분리)·다크=심해(오션→아쿠아, 형광펜 불변) ② `Logo.tsx` 지느러미 6변형(mark/lockup/compact/mono/invert/app; clipPath+useId, "use client")·SVG 4종 재작성(`finnote-mark`·`-mono`·`-icon`·`favicon`, amber-*.svg 삭제) ③ 폰트 교체(Inter Tight 제거→Nunito+Noto Serif KR, Pretendard 유지) ④ 시그니처: 각주 칩(.cite-ref 노랑 evidence)·근거 카드 좌측 하이라이터선+본명조 인용·`.hl/.hl-block/.hl-num`·`.waterline` 구분선·지느러미 로딩(`.fin-swim`)·시장색 KR/US 자동반전(`[data-market]`, `--up/--down`) ⑤ shareCard(canvas)·OG 라우트(satori) 지느러미+두 색 워드마크 재작성 ⑥ 유저 대면 문자열 전수 리네임(웹 10파일·studio 7·admin 3·문서 4; 내부 식별자·vg 쿠키/단축링크·valuegraph 인프라명 유지) | BRAND-1 | ✅ 2026-07-19 — web 99 green (vitest); studio·admin 문자열 리네임 |
